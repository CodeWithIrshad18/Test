...
public async Task<IActionResult> Login([FromBody] AppLogin appLogin)
{
    ...
    using (var client = new HttpClient())
    {
        ...
        // Fetch User Data
        string query = @"SELECT EMA_PERNO, EMA_ENAME, EMA_EMAIL 
        ...
        if (string.IsNullOrEmpty(userLoginData.EMA_EMAIL))
        ...
        string body = $"<h3>Hello {userLoginData.EMA_ENAME}</h3><p>Your OTP is <b>{otp}</b></p>";

        await SendEmailAsync(userLoginData.EMA_EMAIL, null, null, subject, body);
