...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    console.log("⚡️ Face recognition initializing (no Service Worker)...");
    ...
    // ✅ Load + cache descriptors
    async function loadDescriptor(imageUrl, label) {
        const cacheKey = `faceDescriptor_${label}`;
        const cached = localStorage.getItem(cacheKey);
        if (cached) {
            console.log("Loaded descriptor from cache:", label);
        ...
        if (detection?.descriptor) {
            localStorage.setItem(cacheKey, JSON.stringify(Array.from(detection.descriptor)));
    ...
    async function initFaceRecognition() {
        ...
        // ✅ Parallel load
        const [baseDescriptor, capturedDescriptor] = await Promise.all([
            loadDescriptor(baseImageUrl, userId),
            loadDescriptor(capturedImageUrl, `${userId}_captured`)
        ]);
        ...
        function verifyDescriptor(descriptor, faceMatcher, matchMode, baseDescriptor, capturedDescriptor) {
            ...
            if (matchMode === "both") {
                ...
                if (distToBase < 0.35 && distToCaptured < 0.35) return { success: true };
        ...
        function showSuccessAndCapture() {
            ...
            // ✅ smaller size for faster descriptor creation
            const targetWidth = 320;
            const targetHeight = video.videoHeight * (320 / video.videoWidth);
        ...
        async function captureImageAndSubmit(entryType) {
            ...
            fetch("/TSUISLARS/Face/AttendanceData", {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        Swal.fire("Thank you!", `Attendance Recorded.\nDate & Time: ${now}`, "success").then(() => {
                            stopVideo();
                            location.reload();
                        });
                    } else {
                        Swal.fire("Face Verified, But Error!", "Server rejected attendance.", "error").then(() => {
                            stopVideo();
                            location.reload();
                        });
                    }
        ...
        window.captureImageAndSubmit = captureImageAndSubmit;
...
// ↓ resize base image to 240×240 max before face detection
...
const resizedImg = await faceapi.bufferToImage(scaled);

// then run detection on resizedImg instead of img
const detection = await faceapi
    .detectSingleFace(resizedImg, new faceapi.TinyFaceDetectorOptions({ inputSize: 160 }))
    .withFaceLandmarks(true)
    .withFaceDescriptor();
