...
public class ResumeInfo
{
    ...
    public bool GoToAttachment { get; set; }
}
...
protected void Page_Load(object sender, EventArgs e)
{
    if (!IsPostBack)
    {
        ...
        hfGoToAttachment.Value = resumeInfo.GoToAttachment ? "true" : "false";
...
;WITH Q AS (
    SELECT 
        Q.Id AS QuestionId,
        Q.ModuleID,
        M.SerialNo AS ModuleOrder,
        Q.SeqNo AS QuestionOrder,
        R.QuestionID AS Answered
    FROM App_QuestionMaster Q
...
", con))
    ...
    {
        info.GoToAttachment = ShouldGoToAttachment(userId, info.ModuleId);
    }

    return info;
}
private bool ShouldGoToAttachment(int userId, string moduleId)
{
    string cs = ConfigurationManager.ConnectionStrings["ApplicationServices"].ConnectionString;
...
", con))
    {
        ...
        return remaining == 0; // if all answered, move to next module attachment
    }
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const hfGoToAttachment = document.getElementById("hfGoToAttachment");
    ...
    const goToAttachment = hfGoToAttachment ? hfGoToAttachment.value === "true" : false;
    ...
    if (resumeQuestionId || resumeModuleId) {
        ...
        if (!goToAttachment && questionIndex !== -1) {
